...
test('exporter rejects description outside 24–180 characters', () => {
  assert.throws(() => toPost({ metadata: { ...metadata, description: 'Hecho comprobado.' }, markdown:'x' }), /description must contain 24–180 characters/);
  assert.throws(() => toPost({ metadata: { ...metadata, description: 'x'.repeat(181) }, markdown:'x' }), /description must contain 24–180 characters/);
});
